...
class NetworkManagementToolV5:
    ...
    def clean_output_preserve_integrity(self, text, command=None):
        """
        V11增强版输出清理函数，彻底解决分页符遗留空格问题
        核心原则：只移除分页符，绝不丢失任何实际内容，同时彻底清除分页符遗留的空格
        """
        if not text:
            return text
        ...
        # 第二步：处理锐捷设备的退格符问题
        text = re.sub(r'(\x08)+', '', text)  # 移除退格符
        text = re.sub(r'(\bBS\b\s*)+', '', text)  # 移除BS字符串
        ...
        for line in lines:
            ...
            for pattern in pagination_patterns:
                ...
                if match:
                    ...
                    # 根据前后内容决定保留策略
                    if after_clean.strip():  # 分页符后有有效内容
                        cleaned_line = after_clean
                    elif before_clean.strip():  # 分页符前有有效内容
            ...
            # 额外处理：移除行中残留的大量空格（分页符造成的空格混乱）
            # 检测并清理异常的大量连续空格（超过4个连续空格视为异常）
            if '     ' in cleaned_line:  # 5个或更多连续空格
                # 保留配置的缩进结构，但清理异常的大量空格
                # 将多个连续空格压缩为合理的缩进
                cleaned_line = re.sub(r'( {5,})', ' ', cleaned_line)  # 5个以上空格压缩为1个
    ...
    def parse_device_status_from_logs(self):
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "extract_device_status", found_path
            )
            if spec is None or spec.loader is None:
                messagebox.showerror("导入失败", "无法加载 extract_device_status.py！")
                return
            extract_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(extract_module)
            detect_vendor = extract_module.detect_vendor
            extract_info = extract_module.extract_info
            parse_filename = extract_module.parse_filename
        except Exception as e:
            messagebox.showerror("模块加载失败", f"加载 extract_device_status.py 失败: {e}")
            return

        all_device_status = []
        log_files = [f for f in os.listdir(log_dir) if f.endswith('.log') and 'summary' not in f]

        if not log_files:
            messagebox.showwarning("无日志", "在指定目录中未找到任何设备日志文件。")
            return
        ...
        for log_file in log_files:
            ...
            dev_name, timestamp = parse_filename(log_file)
            ...
            all_device_status.append(status_entry)

        self._show_parsed_status_window(all_device_status)

    def _show_parsed_status_window(self, status_data):
        """
        显示解析后的设备状态，带异常排序和导出功能。
        """
        ...
        def export_parsed_data_to_csv():
            if not status_data:
                messagebox.showwarning("无数据", "没有可导出的解析数据。")
                return
